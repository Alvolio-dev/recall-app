...
import { Globe, Clock, Star, ListOrdered, Loader2, AlertCircle, Info } from "lucide-react";
...
function NewSummaryContent() {
  ...
  const summarize = async (
    ...
    try {
      ...
      if (!savedId) saveToLibrary(newResults);
    } catch (err) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-generate verdict immediately
                    ...
                    <div className="grid grid-cols-3 gap-2 p-3 bg-zinc-50 rounded-xl text-[11px] text-zinc-500">
                      {modes.map((m) => (
                        <div key={m.id} className="text-center">
                          ...
                          <p className="font-medium text-zinc-700">{m.label}</p>
                          <p>{m.description}</p>
                        </div>
                      ))}
                    </div>
